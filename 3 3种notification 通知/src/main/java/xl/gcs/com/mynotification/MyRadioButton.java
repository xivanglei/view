 ...
 * 可以调整drawable尺寸
 * 如果在xml中android:drawableRight 设置掉，系统就已经设置了
 * 所以，自定义属性，app:drawableRight 然后这里设置
 * 位置需要另外调控，必要时要android:drawablePadding设成负数
 */
...
public class MyRadioButton extends android.support.v7.widget.AppCompatRadioButton {
    ...
    public void setCompoundDrawablesWithIntrinsicBounds(Drawable top,Drawable bottom,Drawable left,Drawable right){
        ...
        //TextView里面就是通过这个方法来设置的，必须4个参数，有就放进去，没有就null。所以，如果原生方法设置了drawable就改不了了，只能在这里设进去
